...
const dashboard = {
  ko: {
    ...
    /* ── 백테스팅 전면 리포트 카드 (BacktestReportCard) ──── */
    btReportTitle:       'AI 알고리즘 과거 적중률 검증 리포트',
    btReportSub:         '{n}거래일 가상 시뮬레이션 기반 통계',
    /* 메트릭 1: 예측 적중률 */
    btHitRateLabel:      '예측 적중률',
    btHitRateEnLabel:    'Hit Rate',
    btHitRateTooltip:    '적중률 산출 공식: 최근 60일 전 시점부터 당사 AI 가중치 엔진이 \'매수(Buy)\' 또는 \'적극 매수(Strong Buy)\' 신호를 보냈던 과거 시점들을 전수조사합니다. 신호 발생 후 30일 이내에 주가가 실제로 설정 기준치(+5%) 이상 상승하여 수익 구간을 터치했는지 여부를 수학적으로 추적한 실제 통계 승률입니다.',
    btOutOf:             '/',
    btCorrectLabel:      '회 적중',
    /* 메트릭 2: 평균 최고 수익률 */
    btAvgPeakLabel:      '시그널 후 평균 최고 수익률',
    btAvgPeakEnLabel:    'Avg Peak Gain',
    btAvgPeakDesc:       '보유 기간 30거래일 내 최고 수익률 평균',
    /* 메트릭 3: 시그널 수 */
    btSignalCountLabel:  '검증된 총 시그널',
    btSignalCountEnLabel: 'Total Signals',
    btSignalCountUnit:   '회 구간',
    /* 공통 */
    btNoSignalMsg:       '탐색 구간 내 매수 시그널 없음 — 데이터 추가 후 재시도하세요.',
    btSimNote:           '가상 시뮬레이션 기반 통계 · 연간 재무 데이터(Piotroski, PEG)는 현재값 고정 사용 · 실제 투자 결과와 다를 수 있습니다.',
    /* 국면 조정 점수 (AIScoreWidget 하단 패널) */
    btHoldingDays:       '30거래일 보유 기간',
    btThreshold:         '적중 기준 +5%',
  ...
  en: {
    ...
    secBacktest:   'AI Algorithm Hit Rate Validation',
    ...
    /* ── Backtest Badge (AIScoreWidget header) ───────────── */
    ...
    /* ── BacktestReportCard ──────────────────────────────── */
    btReportTitle:       'AI Algorithm Hit Rate Validation Report',
    btReportSub:         '{n}-day virtual simulation statistics',
    /* Metric 1: Hit Rate */
    btHitRateLabel:      'Predicted Hit Rate',
    btHitRateEnLabel:    'Hit Rate',
    btHitRateTooltip:    "Hit Rate Formula: We audit all past 'Buy' or 'Strong Buy' signals generated by our AI weighting engine starting from 60 days ago. This percentage represents the historical win rate where the stock price actually surged above the target threshold (+5%) within 30 trading days post-signal.",
    btOutOf:             '/',
    btCorrectLabel:      'correct',
    /* Metric 2: Avg Peak Gain */
    btAvgPeakLabel:      'Avg Peak Return Post-Signal',
    btAvgPeakEnLabel:    'Avg Peak Gain',
    btAvgPeakDesc:       'Peak gain within 30 trading days of signal',
    /* Metric 3: Signal Count */
    btSignalCountLabel:  'Total Verified Signals',
    btSignalCountEnLabel: 'Total Signals',
    btSignalCountUnit:   'signal events',
    /* Common */
    btNoSignalMsg:       'No buy signals found in the search window — retry after more data is available.',
    btSimNote:           'Virtual simulation statistics · Annual fundamental data (Piotroski, PEG) fixed at current values · Actual results may differ.',
    /* Dynamic score panel */
    btHoldingDays:       '30-day holding period',
    btThreshold:         '+5% touch threshold',
